...
class Attention(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # self.c_proj = lora.Linear(cfg.n_embd, cfg.n_embd, cfg.rank*2)
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            # mask 的形狀應為 [B, 1, T, T] 或 [B, n_head, T, T]
            mask = mask.unsqueeze(1)
            mask = mask.expand(-1, 12, -1, -1)
...
class Decoder(nn.Module):
    ...
    def forward(self, x: Tensor, img_embeddings: Tensor):
        ...
        I_B, I_L, I_dim = img_embeddings.shape
        x = torch.narrow(x, 1, 0, min(x.size(1), self.block_size))
        # x = self.transformer.wte(x) + self.transformer.wpe(pos)
        x = self.transformer.wte(x)
        T_B, T_L, T_dim = x.shape
        ...
        # if self.training:
        #     img_mask = torch.ones((T_B, T_L+I_L, T_L+I_L), dtype=torch.bool)
        #     img_mask_img = torch.zeros((I_L, T_L), dtype=torch.bool)
        #     causal_mask = torch.tril(torch.ones((T_L, T_L), dtype=torch.bool))
        #     # for i in range(T_B):
        #     img_mask[:, :I_L, I_L:] = img_mask_img.unsqueeze(0).expand(T_B, -1, -1)
        #     img_mask[:, I_L:, I_L:] = causal_mask.unsqueeze(0).expand(T_B, -1, -1)
        #     # print(img_mask.shape)
        #     img_mask = img_mask.to(x.device)
        # else:
        #     img_mask=None
        img_mask=None
        # # TODO here change
        x, m = self.transformer.h((x, img_mask))
        x = self.transformer.ln_f(x)
        x = self.lm_head(x)
        # x = self.lm_head(self.transformer.ln_f(self.transformer.h(x, img_mask)))
        return x
